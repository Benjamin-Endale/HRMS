...
const MainBody = ({readPath}) => {
  return (
        ...
        <aside className=' customBorder scrollBar w-[20.5rem]  h-screen flex flex-col gap-[4.25rem] relative  pt-[3.5rem] overflow-y-auto font-semibold'>
            ...
            <nav className='flex flex-col gap-[4.0625rem] overflow-y-auto scrollBar '>
                ...
                <section className='space-y-[1.5625rem] w-full  pl-[2.75rem] relative'> 
                    <div>
                        <h4 className={`${['Dashboard' , 'organization' , 'employees', 'attendance' , 'leaveManagment' ,"AddNewemployee"].includes(readPath) ? 'text-lemongreen' : 'text-limegray'} text-[0.9375rem]`}>CORE</h4>
                    </div>
    ...
    <path d="M8.75 12.25H12.25"  stroke-linecap="round"/>
                    ...
                    {/* Employee */}
                    <div className='flex items-center   '>
                        <div  className={`${readPath === 'employees' || readPath === 'AddNewemployee' || readPath === 'AddNewemployeesecond' ? 'flex' : 'hidden'} absolute  left-0  navBarhover `}></div>
                        <div className='navLinkconfig'> 
                            <svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg" stroke={readPath === 'employees' || readPath === 'AddNewemployee' || readPath === 'AddNewemployeesecond' ? 'white' : '#5D6150'} stroke-width="1.5">
                            ...
                            <div>
                                <NavLink to='/employees'><h4 className={`${readPath === 'employees' || readPath === 'AddNewemployee' || readPath === 'AddNewemployeesecond' ? 'text-white' : 'text-limegray'}`}>Employees</h4></NavLink>
                            </div>
                        </div>
                    </div>
                    {/* Attendance */}
